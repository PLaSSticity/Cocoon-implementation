...
use secret_structs::secret_block;
...
#[derive(Default, InvisibleSideEffectFreeDerive)]
struct Player<L: lat::Label> {
    ship_positions: st::Secret<Grid<bool>, L>,
...
// Need to add derive macro for enums too.
unsafe impl st::InvisibleSideEffectFree for Ship {}
unsafe impl st::InvisibleSideEffectFree for CellStatus {}
...
fn main() {
    session_types::connect(
        |chan| {
            let pb = Player::<lat::Label_B>::new();
            game_loop_b(pb, chan);
        },
        |chan| {
            let pa = Player::<lat::Label_A>::new();
            game_loop_a(pa, chan);
        },
    );
}
...
fn place_ship(grid: &mut Grid<bool>, placement: &Placement) {
    ...
    while row < placement.start_row + placement.size && col < placement.start_col + placement.size {
        unchecked_operation(grid[row][col] = true);
        row += row_step;
        col += col_step;
    }
}

impl<L: lat::Label> Player<L> {
    fn new() -> Player<L> {
        let ship_positions = secret_block!(L {
            let ships = [Ship::Carrier, Ship::Battleship, Ship::Cruiser,
...
fn game_loop_b(mut player: Player<lat::Label_B>, chan: session_types::Chan<(), PlayerB>) {
    let mut c = chan.enter();
    ...
    loop {
        ...
        let is_hit: bool = secret_block!(lat::Label_B {
            wrap_secret(is_occupied(unwrap_secret_ref(&player.ship_positions), guess.0, guess.1))
        })
        .declassify()
        .get_value_consume();
